...
<head>
    ...
    <style>
        ...
        .table thead th:nth-last-child(-n+2):hover {
            background: #232528;
        }
    ...
    <div class="page-container">
        ...
        <div class="content-container">
            <div class="header-actions">
                ...
                <a href='/admin/view-students'><button class="btn btn-primary">👥 View Students</button></a>
            </div>
            
            @if($students->count() > 0)
            <div class="search-bar">
                <input type="text" id="searchInput" placeholder="🔍 Search by name, email, address, student no, grade, section, or mark..." onkeyup="searchTable()">
            </div>
                        ...
                        @foreach($students as $student)
                            <tr>
                                ...
                                <td class="text-center">
                                    @if(in_array($student->id, $dropped))
                                        <span class="badge bg-danger">Dropped</span>
                                    @else
                                        <form method="POST" action="{{ route('admin.mark-drop', $student->id)}}">
                                            @csrf
                                            <button type="button" class="btn btn-sm btn-danger"
                                            data-bs-toggle="modal" data-bs-target="#dropModal{{ $student->id }}">Mark as Dropped</button>

                                            <div class="modal fade" id="dropModal{{ $student->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                            <div class="modal-dialog modal-dialog-centered">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h1 class="modal-title fs-5" id="exampleModalLabel">Drop Student</h1>
                                                </div>
                                                <div class="modal-body">
                                                    Proceed to drop student {{ $student->lname }}, {{ $student->fname }}?
                                                    <br>Once dropped, you cannot unmark this student anymore
                                                </div>
                                ...
                                <td class="d-flex gap-2 justify-content-center">
                                    <form action="/admin/restore-student/{{ $student->id }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button class="btn border-success" type="button" title="Restore" data-bs-toggle="modal"
                                        data-bs-target="#delStudModal{{ $student->id }}">↺</button>

                                        <div class="modal fade" id="delStudModal{{ $student->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-centered">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h1 class="modal-title fs-5" id="exampleModalLabel">Restore Student</h1>
                                                </div>
                                                <div class="modal-body">
                                                    Proceed to restore student {{ $student->lname }}, {{ $student->fname }}?
                                                </div>
                                                <div class="modal-footer">
                                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
