    ...
    <div class="container">
        <div class="content-container">
            <div class="header-actions">
                ...
                <a href="/admin/view-inst-subjects"><button class="btn btn-success">View Assigned Subjects</button></a>
            </div>
            ...
            <div class="search-bar">
                <input type="text" id="searchInput" placeholder="🔍 Search by name, email, address, grade level, or section..." onkeyup="searchTable()">
            </div>
                ...
                <table class="table table-bordered" id="instructorTable">
                    <thead>
                        <tr>
                            ...
                            <th onclick="sortTable(3)">Grade Level <span class="sort-icon">⬍</span></th>
                            <th onclick="sortTable(4)">Section <span class="sort-icon">⬍</span></th>
                            <th>Actions</th>
                        </tr>
                    </thead>
                    <tbody id="tableBody">
                        ...
                        <tr>
                            <td>{{ $instructor->lname }}, {{ $instructor->fname }}</td>
                            <td>{{ $instructor->email }}</td>
                            <td>{{ $instructor->address }}</td>
                            <td style="text-align: center;">{{ $instructor->section->grade->roman_numeral }}</td>
                            <td>{{ $instructor->section->name }}</td>
                            <td class="d-flex gap-2 justify-content-center">
                                <form action="{{ route('admin.delete-instructor', $instructor->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button class="btn border-danger" type="button" title="Delete"
                                    data-bs-toggle="modal" data-bs-target="#deleteInst{{$instructor->id}}">🗑️</button>

                                    <!-- Modal -->
                                    <div class="modal fade" id="deleteInst{{$instructor->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                    <div class="modal-dialog modal-dialog-centered">
                                        <div class="modal-content">
                                        <div class="modal-body">
                                            Proceed to delete instructor {{ $instructor->lname }}, {{ $instructor->fname }}?
                                            <br>This will delete the instructor's user account.
                                        </div>
